 ...
 */
(function ($) {
    let ColorHex = ['00', '33', '66', '99', 'CC', 'FF'];
    let SpColorHex = ['FF0000', '00FF00', '0000FF', 'FFFF00', '00FFFF', 'FF00FF'];
    $.fn.colorpicker = function (options) {
        let opts = jQuery.extend({}, jQuery.fn.colorpicker.defaults, options);
        initColor();
        return this.each(function () {
            let obj = $(this);
            obj.bind(opts.event, function () {
                //定位
                let ttop = $(this).offset().top;     //控件的定位点高
                let thei = $(this).height();  //控件本身的高
                let tleft = $(this).offset().left;    //控件的定位点宽
                $("#colorpanel").css({
                    top: ttop + thei + 5,
                    left: tleft
                }).show();
                $("#colorpanel").css('z-index', 100000);
                let target = opts.target ? $(opts.target) : obj;
